...
const createReview = async function (req, res) {
  try {
    ...
    if (savedData) {
      let newReview = await booksModel.findOneAndUpdate(
        { _id: bookId },
        {
          $inc: {
            reviews: 1,
    ...
    let check1 = await booksModel
      .findOne({ _id: bookId, isDeleted: false })
      .select({ deletedAt: 0, __v: 0, ISBN: 0 })
      .lean();
...
const updateReview = async function (req, res) {
  try {
    ...
    let check1 = await reviewModel.findOne({ _id: reviewId,bookId:bookId, isDeleted: false })
    if (!check1)
      return res.status(404).send({ status: false, msg: "reviews not found" });
    let data = req.body;
    if (!validation.isValidRequestBody(data)) {
      return res
        .status(400)
    ...
    if (data.reviewedBy) {
      if (!validation.isValidString(data.reviewedBy)) {
        return res
          .status(400)
...
const deleteReviewsbyId = async function (req, res) {
  try {
    ...
    if (updateReview) {
      let deleteReview = await booksModel.findOneAndUpdate(
    ...
    if (!updateReview)
      return res
        ...
        .send({
          status: false,
          message: "Invalid request : Please enter correct bookId or reviewId",
        });
    res
      .status(200)
      .send({ status: true, message: "review is deleted successfully" });
  } catch (err) {
    console.log(err);
